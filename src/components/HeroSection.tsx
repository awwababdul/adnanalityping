...
const HeroSection = () => {
  ...
  return (
    <div className="relative min-h-[90vh] overflow-hidden bg-gradient-to-br from-primary/95 to-primary">
        ...
        >
          ...
          >
            ...
            <img 
              ...
              className="w-36 h-36 mx-auto relative drop-shadow-2xl"
            />
          </motion.div>

          <motion.h1 
            className="text-4xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-white mb-8 tracking-tight leading-none text-balance"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.4, duration: 0.8, ease: "easeOut" }}
          >
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-blue-100 to-white drop-shadow-sm">
          ...
          >
            {["24+ Years of Experience", "GDRFA, DHA & EIDA Authorized", "Fastest Turnaround", "Total Confidentiality", "Multilingual Support"].map((tag, index) => (
              <motion.div 
                key={index} 
                className="flex items-center glass px-4 py-2 rounded-full backdrop-blur-md border border-white/20 hover:bg-white/15 transition-all duration-300"
          ...
          >
            <Button 
              ...
              className="w-full sm:w-auto px-8 bg-gradient-to-r from-success to-success-600 hover:shadow-glow-lg hover:shadow-success/30 font-semibold"
            >
          ...
          <motion.div 
            className="max-w-2xl mx-auto mb-12 glass-card p-8 rounded-2xl border border-white/10"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="relative mb-6">
              <Input
                ...
                className="w-full h-16 pl-14 pr-4 rounded-2xl text-lg bg-white/15 text-white placeholder:text-blue-200 border-white/20 focus-visible:ring-2 focus-visible:ring-white/30 backdrop-blur-md font-medium"
                value={searchQuery}
              ...
              <Button 
                className="absolute right-2 top-1/2 transform -translate-y-1/2 rounded-xl h-12 px-6 font-semibold" 
                onClick={handleSearchClick}
              >
                Find Services
              </Button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { label: "Visa Services", query: "Visa" },
                { label: "Emirates ID", query: "Emirates ID" },
                { label: "Business Setup", query: "Business" }
              ].map((item, index) => (
                <Button 
                  key={item.label}
                  variant="ghost" 
                  className="text-white hover:bg-white/15 border border-white/10 rounded-xl h-12 font-medium transition-all duration-300" 
          ...
          >
            <Link
              to="/upload-documents"
              className="flex items-center gap-3 glass px-6 py-3 rounded-2xl hover:bg-white/15 transition-all duration-300 hover:scale-105 font-medium"
            >
            ...
            <a 
              ...
              className="flex items-center gap-3 hover:text-white transition-colors duration-300 font-medium"
            >
